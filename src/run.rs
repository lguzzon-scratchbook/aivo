...
use crate::cli::{AccountSubcommand, Cli, CodeArgs, Commands, ModelsArgs, RunArgs, ServeArgs};
use crate::cli_args::{
...
pub async fn run() -> ! {
    ...
    let exit_code = match command {
        ...
        Commands::Code(code_args) => dispatch_code(code_args, session_store, models_cache).await,

        Commands::Run(run_args) => dispatch_run(run_args, session_store, models_cache).await,

        Commands::Models(models_args) => {
            dispatch_models(models_args, session_store, models_cache).await
        }

        Commands::Serve(serve_args) => dispatch_serve(serve_args, session_store).await,
...
/// `aivo code`: validates the sandbox/schema/root flags, splits `[key::]model`,
/// resolves the key, then hands the session to `CodeCommand`.
async fn dispatch_code(
    mut code_args: CodeArgs,
    ...
    if let Some(spec) = code_args.json_schema.take() {
        ...
        let schema = match spec.strip_prefix('@') {
            Some(path) => {
                match std::fs::read_to_string(crate::services::system_env::expand_tilde(path)) {
                    Ok(s) => s,
                    Err(e) => {
...
/// `aivo run <tool>`: recovers aivo flags clap swallowed into passthrough,
/// resolves key/model/slots, then launches the tool (or the `start` picker).
async fn dispatch_run(
    ...
    let ai_launcher = AILauncher::new(session_store.clone(), env_injector, models_cache.clone());
    ...
    let is_pi = run_args.tool.as_deref().and_then(AIToolType::parse) == Some(AIToolType::Pi);
    ...
    if run_args.tool.is_none() || code_selected {
        ...
        let command = RunCommand::new(session_store.clone(), ai_launcher, models_cache.clone());
        ...
        let persisted_model_for_key = if model_flag_explicit {
            ...
            match (persisted_model_for_key, key_override.as_ref()) {
                ...
                {
                    if services::model_catalog::starter_model_still_available(k, &models_cache, &m)
                        .await
                    {
                        Some(m)
                    } else {
...
/// `aivo serve`: an HF positional takes over the key slot with a loopback
/// llama-server; otherwise the key is optional (`MissingAuth` is a valid mode).
async fn dispatch_serve(serve_args: ServeArgs, session_store: SessionStore) -> ExitCode {
