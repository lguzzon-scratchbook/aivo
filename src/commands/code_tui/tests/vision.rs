...
body into the target type: messages[0]: unknown variant `image_url`, expected `text` at line 1\"}}"
    ...
    assert!(is_image_input_rejection(
        "LLM error: upstream 400 Bad Request: {\"error\":{\"message\":\"This model does not \
support image\",\"type\":\"invalid_request_error\"}}"
    ));
    assert!(!is_image_input_rejection("rate limit exceeded"));
    assert!(!is_image_input_rejection("unknown variant `foo`"));
}

/// Tool images live only in engine messages, so after a key/model switch the
/// stashed transcript is the pre-send scan's sole view of them — a miss sends
/// the raw image to a text-only provider.
#[test]
fn stashed_transcript_images_are_visible_to_the_presend_scan() {
    let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
    let mut app = make_test_app(tx, rx);
    assert!(!app.pending_messages_have_images(), "no stash yet");

    app.pending_agent_messages = Some(vec![
        serde_json::json!({"role": "user", "content": "make an image"}),
        serde_json::json!({"role": "user", "content": [
            {"type": "text", "text": "Image generated by `generate_image` (saved to /tmp/gen-x.jpg):"},
            {"type": "image_url", "image_url": {"url": "data:image/jpeg;base64,aGk="}},
        ]}),
    ]);
    assert!(app.pending_messages_have_images());

    app.pending_agent_messages = Some(vec![
        serde_json::json!({"role": "assistant", "content": "text only"}),
    ]);
    assert!(!app.pending_messages_have_images(), "text-only stash");
}
