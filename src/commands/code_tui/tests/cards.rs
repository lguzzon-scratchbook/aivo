...
fn test_permission_card_anchored_above_composer() {
    ...
    // The card hugs the composer from its reserved slot: bottom border, the
    // composer's breathing row, then the input's rounded top border directly
    // above the prompt (not floating mid-screen like a centered modal).
    ...
    assert_eq!(
        bottom_border_row + 3,
        composer_row,
        "card bottom must sit just above the composer (breathing row + top border):\n{screen}"
    );
    // The border two rows under the card is the full-width composer top — now
    ...
    let divider = &rows[bottom_border_row + 2];
    assert!(
