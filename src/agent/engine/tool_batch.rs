...
Investigate, or call `exit_plan_mode` with your plan."
        ...
        if !parallel_idx.is_empty() {
            ...
            for (i, result) in futures::future::join_all(runs).await {
                ...
                outcomes[i] = Some(absorb_tool_output(
                    result,
                    names[i],
        ...
        if !batch_images.is_empty() && self.model_reads_images {
            ...
            for (tool, images) in batch_images {
                for mut img in images {
                    parts.push(json!({
                        "type": "text",
                        "text": format!("Image from `{tool}` ({}):", img.path.display()),
                    }));
                    // Shrink for the wire — full-res stays at the saved path.
                    if let Some((mime, data)) =
                        crate::services::image_optimize::optimize_base64(&img.data_b64)
                    {
                        img.mime = mime;
                        img.data_b64 = data;
                    }
...
(~/.ssh or ~/.gnupg). Re-run with no confinement at all?"
    ...
    pub(super) async fn run_write_with_escalation(
        ...
        {
            return tools::execute(name, args, ctx.cwd).await.map(|o| o.text);
        }
...
/// Mid-turn `image_url` 400s a text-only model (the vision shim is pre-turn only).
fn absorb_tool_output(
    result: Result<crate::agent::engine::ToolOutput, String>,
    tool: &str,
    model_reads_images: bool,
    batch_images: &mut Vec<(String, Vec<crate::agent::engine::ToolImage>)>,
) -> Result<String, String> {
    let mut out = result?;
    if out.images.is_empty() {
        return Ok(out.text);
    ...
    if !model_reads_images {
        out.text
            .push_str("\n[Current model does not support images. The image was not sent.]");
        return Ok(out.text);
    }
    batch_images.push((tool.to_string(), out.images));
    Ok(out.text)
}
