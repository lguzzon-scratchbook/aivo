...
/// Rides the tail of every plan-mode request (ephemeral, never persisted): the
/// system-prompt directive decays over a long conversation, and a user "go ahead"
/// otherwise tempts the model into executing while read-only.
